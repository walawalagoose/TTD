...
class Rotta(BaseAdaptation):
    ...
    def _preprocess_image(self, x):
        ...
        if x.shape[2] == self.input_resolution:
            ...
            x = x.unsqueeze(0)
            x = nn.functional.interpolate(x, size=(self.input_resolution, self.input_resolution), 
                            mode='bilinear', align_corners=False)
            return x.squeeze(0)
        elif len(x.shape) == 4:
            return nn.functional.interpolate(x, size=(self.input_resolution, self.input_resolution), 
                            mode='bilinear', align_corners=False)
        else:
        ...
        with timer("forward"):
            with torch.no_grad():
                if "vit" in self._meta_conf.model_name:
                    batch._x = self._preprocess_image(batch._x)
                model.eval()
                self.model_ema.eval()
    ...
    def update_model(self, model, optimizer):
        ...
        if len(sup_data) > 0:
            ...
            if "vit" in self._meta_conf.model_name:
                sup_data = self._preprocess_image(sup_data)
                strong_sup_aug = self._preprocess_image(strong_sup_aug)
            ema_sup_out = self.model_ema(sup_data)
            stu_sup_out = model(strong_sup_aug)
            instance_weight = self.timeliness_reweighting(ages).to(stu_sup_out.device)
