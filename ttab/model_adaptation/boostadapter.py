...
class BoostAdapter(BaseAdaptation):
    """
    ...
    def __init__(self, meta_conf, model: nn.Module):
        clip_model = ClipZeroShot4TTA(
            ...
            arch="ViT-L/14")
        super(BoostAdapter, self).__init__(meta_conf, clip_model)
    ...
    def _initialize_model(self, model: nn.Module):
        ...
        # it'ok to set all modules in eval mode, since prompt learner in boostadapter has no BN layers or Dropout.
        model.eval()
    ...
    def update_cache(self, cache, pred, features_loss, shot_capacity, include_prob_map=False, fifo=True):
        with torch.no_grad():
    ...
    def get_clip_logits(self, image_features, clip_logits, infer_ori_image=False):
            ...
            if image_features.size(0) > 1:
                if infer_ori_image:
                    ...
                    loss = adaptation_utils.softmax_entropy(clip_logits[:1])
    ...
    def one_adapt_step(
        ...
        with timer("forward"):
            # Data augmentation
            ori_input = self._preprocess_image(batch._x)
            aug_inputs = [
                self._preprocess_image(
                    self.transform_helper(batch._x, data_name=self._meta_conf.base_data_name)
                    )
            ...
            image_features, clip_logits, loss, prob_map, pred, ori_feat, ori_output = self.get_clip_logits(image_features, clip_logits, self._meta_conf.infer_ori_image)
            ...
            if self.pos_config['enabled']:
                ...
                select_feat, select_output, select_idx = select_confident_samples(ori_feat, ori_output, self._meta_conf.rou)
                ...
                for i in range(select_entropy.shape[0]):
                        ...
                        self.update_cache(
                            ...
                            self.pos_config['shot_capacity'] + self._meta_conf.delta, fifo=False)
