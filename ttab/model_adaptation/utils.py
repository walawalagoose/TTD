...
def normalize_logits(logits, eps=1e-6):
...
def teacher_student_mutual_information(
    x: torch.Tensor, x_ema: torch.Tensor, eps: float = 1e-8
) -> torch.Tensor:
    """
    Computes the mutual information between teacher and student predictions over a batch.
    """
    # Step 1: Convert logits to probabilities
    p_stud = F.softmax(x, dim=1)      # Shape: (N, C)
    p_teach = F.softmax(x_ema, dim=1) # Shape: (N, C)
    
    # Get batch size
    N = p_stud.shape[0]

    # Step 2: Estimate marginal distributions by averaging over the batch
    # p(y_stud)
    p_stud_marginal = torch.mean(p_stud, dim=0)   # Shape: (C)
    # p(y_teach)
    p_teach_marginal = torch.mean(p_teach, dim=0) # Shape: (C)

    # Step 3: Estimate the joint distribution p(y_stud, y_teach)
    # The (i, j) element of the joint probability matrix is the probability
    # that teacher predicts class i AND student predicts class j.
    # We estimate this by averaging the outer product of probabilities over the batch.
    # A fast way to do this is with matrix multiplication.
    p_joint = (p_teach.T @ p_stud) / N   # Shape: (C, C)

    # Step 4: Compute mutual information using the formula
    # I(X;Y) = sum_{x,y} p(x,y) * log( p(x,y) / (p(x) * p(y)) )
    
    # Calculate the product of marginals p(x) * p(y)
    # We need to reshape marginals to (C, 1) and (1, C) for broadcasting
    p_independent = p_teach_marginal.view(-1, 1) * p_stud_marginal.view(1, -1) # Shape: (C, C)
    
    # Add epsilon for numerical stability (to avoid log(0) and division by 0)
    mi = torch.sum(p_joint * torch.log(p_joint / (p_independent + eps) + eps))
...
def IID_loss(x_out, x_tf_out, lamb=1.0, eps=1e-10):
    """Calculate the IID (Information Invariant Decoding) loss with numerical stability improvements.
    
    Args:
        x_out: Output probabilities of shape [batch_size, num_classes]
        x_tf_out: Transformed output probabilities of same shape
        lamb: Weight factor for marginal entropy terms
        eps: Small constant for numerical stability
    """
    # Check inputs have been softmaxed
    bn_, k_ = x_out.size()
    assert (x_tf_out.size(0) == bn_ and x_tf_out.size(1) == k_)
    
    # Compute joint distribution p(i,j)
    p_i_j = torch.clamp(x_out.unsqueeze(2) * x_tf_out.unsqueeze(1), min=eps)  # [bn,k,k]
    p_i_j = p_i_j.sum(dim=0)  # [k,k] 
    p_i_j = (p_i_j + p_i_j.t()) / 2.  # Symmetrize
    p_i_j = p_i_j / (p_i_j.sum() + eps)  # Normalize
    
    # Compute marginals
    p_i = torch.clamp(p_i_j.sum(dim=1).view(k_, 1), min=eps)  # [k,1]
    p_j = torch.clamp(p_i_j.sum(dim=0).view(1, k_), min=eps)  # [1,k]
    
    # Expand marginals
    p_i = p_i.expand(k_, k_)
    p_j = p_j.expand(k_, k_)
    
    # Compute loss with stable log
    loss = - p_i_j * (
        torch.log(p_i_j + eps) 
...
def calculate_plpd(x, model, aug_type='patch', patch_len=4, occlusion_size=112, row_start=56, column_start=56):
    """
    计算Predicted Label Probability Drop (PLPD)
    
    Args:
        x: 输入图像，形状为[B, C, H, W]
        model: 模型
        aug_type: 数据增强类型，可选'occ', 'patch', 或'pixel'
        patch_len: 用于'patch'增强的块大小
        occlusion_size: 用于'occ'增强的遮挡窗口大小
        row_start, column_start: 用于'occ'增强的窗口起始位置
        
    Returns:
        plpd: 形状为[B]的张量，表示每个样本的PLPD值
        x_prime: 增强后的图像
    """
    # 进行前向传播获取原始输出
    with torch.no_grad():
    ...
    if aug_type == 'occ':
        ...
        occlusion_window = final_mean.expand(-1, -1, occlusion_size, occlusion_size)
        x_prime[:, :, row_start:row_start+occlusion_size, column_start:column_start+occlusion_size] = occlusion_window
    elif aug_type == 'patch':
        # 块重排列增强
        import torchvision
...
def gentropy_loss(softmax_out, eps=1e-5):
    softmax_out = softmax_out.softmax(dim=-1)
    # msoftmax = softmax_out.mean(dim=0)
    msoftmax = torch.clamp(softmax_out.mean(dim=0), min=eps)
    gentropy_loss = -torch.sum(msoftmax * torch.log(msoftmax))
