...
class Pseudo(BaseAdaptation):
    def __init__(self, meta_conf, model: nn.Module):
        super(Pseudo, self).__init__(meta_conf, model)
    ...
    def _initialize_trainable_parameters(self):
        ...
        ), "Pseudo needs some adaptable model parameters."
    ...
    def one_adapt_step(
        ...
        with timer("forward"):
            ...
            # use hard pseudo-labels for cross-entropy
            pseudo_targets = y_hat.detach().argmax(dim=1)
            loss = nn.functional.cross_entropy(y_hat, pseudo_targets).mean(0)
