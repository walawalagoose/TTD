...
class RuntimeTracker(object):
    ...
    def reset(self):
        ...
        # 新增阶段性统计
        self.periodic_stat = dict((name, AverageMeter()) for name in self.metrics_to_track)

    def add_stat(self, metric_name: str):
        self.stat[metric_name] = AverageMeter()
        # 新增阶段性统计
        self.periodic_stat[metric_name] = AverageMeter()
        if metric_name not in self.metrics_to_track:
            self.metrics_to_track.append(metric_name)
    ...
    def update_metrics(self, metric_stat, n_samples):
        for name, value in metric_stat.items():
            ...
            self.periodic_stat[name].update(value, n_samples) # 新增阶段性统计
    ...
    def get_and_reset_periodic_stats(self):
        """获取当前统计并重置阶段性计数，但保留总体统计"""
        periodic_stats = {}
        for name in self.metrics_to_track:
            periodic_stats[name] = self.periodic_stat[name].avg
            self.periodic_stat[name].reset()  # 重置阶段性统计
