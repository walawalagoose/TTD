...
class MergeMultiTestDatasets(object):
    ...
    # NEW: shuffle merged dataset by "domain block -> batch block -> shuffle batch order -> flatten"
    # from DPCore: https://github.com/yunbeizhang/DPCore
    def _inter_batch_shuffle_merged_dataset(
        self,
        merged_dataset: PyTorchDataset,
        per_domain_lengths: List[int],
        batch_size: int,
        ...
        # 计算每个域在 merged_dataset 中的起止偏移（按顺序拼接）
        offsets = []
        s = 0
        for L in per_domain_lengths:
            offsets.append((s, s + L))
            s += L

        # 生成每个域的 batch 块（连续全局索引），不丢弃最后一小块
        domain_blocks: List[List[List[int]]] = []
        for start, end in offsets:
            domain_indices = list(range(start, end))
            blocks = [domain_indices[i : i + batch_size] for i in range(0, len(domain_indices), batch_size)]
            domain_blocks.append(blocks)

        if mode == "random":
            # 直接把所有块展平后按块随机打乱
            batches = [blk for blocks in domain_blocks for blk in blocks]
            rng = random.Random(random_seed) # 打乱 batch 顺序
            rng.shuffle(batches)
            new_indices = [idx for block in batches for idx in block] # 展平成新的索引顺序
            merged_dataset.replace_indices(indices_pattern="new", new_indices=new_indices)
            return merged_dataset
        
        elif mode == "dirichlet":
            # 按 Dirichlet 比例把各域的 batch 块分配到若干 slot，各 slot 内打乱后串接
            # Different from _intra_non_iid_shift, which is applied on class distribution
            rs = np.random.RandomState(random_seed)
            num_domains = len(domain_blocks)
            label_distribution = rs.dirichlet([delta] * slot_num, size=num_domains)  # (D, S)

            slot_blocks: List[List[List[int]]] = [[] for _ in range(slot_num)]
            for d, blocks in enumerate(domain_blocks):
                n = len(blocks)
                if n == 0:
                    continue
                props = label_distribution[d]  # 长度 slot_num，和为1
                ...
                for s_idx, sz in enumerate(sizes):
                    if sz > 0:
                        slot_blocks[s_idx].extend(blocks[cursor : cursor + sz])
                        cursor += sz

            # 各 slot 内按块打乱，再按 slot 顺序串接
            rng = random.Random(random_seed)
            ordered_blocks: List[List[int]] = []
            for s_idx in range(slot_num):
                rng.shuffle(slot_blocks[s_idx])
                ordered_blocks.extend(slot_blocks[s_idx])

            new_indices = [idx for block in ordered_blocks for idx in block]
            merged_dataset.replace_indices(indices_pattern="new", new_indices=new_indices)
            return merged_dataset
        
        else:
            raise NotImplementedError(f"invalid cdc_mode={mode}.")
    ...
    def merge(
        ...
        random_seed: int = None,
        cdc_mode: str = None,
        cdc_slot_num: int = None,
        cdc_delta: float = None,
    ) -> PyTorchDataset:
        # NEW: maybe cross-domain batch shuffle
        def maybe_cross_domain_batch_shuffle(merged_ds, per_domain_ds_list):
            if getattr(test_case, "cross_domain_batch_shuffle", False) and test_case.data_wise == "batch_wise":
                per_domain_lengths = [len(ds) for ds in per_domain_ds_list]
                return self._inter_batch_shuffle_merged_dataset(
                    merged_ds, per_domain_lengths, test_case.batch_size, random_seed, cdc_mode, cdc_slot_num, cdc_delta
                )
            return merged_ds
        ...
        if isinstance(test_case.inter_domain, HomogeneousNoMixture):
            ...
            merged = self._merge_datasets(per_domain_ds)
            return maybe_cross_domain_batch_shuffle(merged, per_domain_ds)
...
class ConstructTestDataset(object):
    ...
    def _get_imagenet_test_domain_datasets_helper(
        ...
        dataset = ImageNetDataset(
            ...
            root=self.data_path, # modified: directly point to the imagenet folder
            data_name=test_domain.data_name,
            split=split,
            device=self.device,
            data_augment=data_augment,
        ...
        # # The transform stats in some CTTA+ViT cases are inconsistent with the version of timm, which might harm the model performance.
        # # so, for ViT+ImageNet, we align with timm's setting.
        # if 'vit' in self.meta_conf.model_name:
        #     import timm
        #     net = timm.create_model(self.meta_conf.model_name, pretrained=True)
        #     dataset.transform = timm.data.create_transform(**timm.data.resolve_model_data_config(net), is_training=False)
        #     del net # only a temporary variable
        return dataset
    ...
    def construct_test_dataset(
        ...
        return MergeMultiTestDatasets().merge(
            ...
            random_seed=self.seed,
            cdc_mode=getattr(self.meta_conf, "cdc_mode", None),
            cdc_slot_num=getattr(self.meta_conf, "cdc_slot_num", None),
            cdc_delta=getattr(self.meta_conf, "cdc_delta", None),
        )
