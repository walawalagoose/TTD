...
# from torch._utils import _accumulate # torch v2.3.0 no longer supports
import bisect
...
class CIFARDataset(PyTorchDataset):
    ...
    def _load_deterministic_cifar_c(
        ...
        domain_path = os.path.join(os.path.abspath('.') , root + "_c", shift_name + ".npy") # hjz modified
    ...
    def __init__(
        ...
        if issubclass(data_shift_class.func, NoShiftedData):
            if "deterministic" in data_name:
                ...
                # validdir = os.path.join(root, "val") # hjz modified: define "imagenet" as in-distribution data
                validdir = os.path.join(root, "imagenet")
                dataset = ImageFolderDataset(
                    root=validdir,
                    transform=self.transform,
                    target_transform=self.target_transform,
...
class MergeMultiDataset(PyTorchDataset):
    ...
    def __init__(self, datasets: List[PyTorchDataset]):
        ...
        # 新增：适配CTTA，记录每个子数据集长度，以确保每个batch只包含该数据集
